...
const ERROR_REASONS: Record<string, string> = {
  'ERROR:INVALID_EXPANSION':     'Invalid expansion — you may not add new disk(s) and remove existing disk(s) at once.',
  'ERROR:NEW_DISK_TOO_SMALL':    'The replacement disk must be as big or bigger than the original.',
  'ERROR:PARITY_NOT_BIGGEST':    'Disk in the parity slot is not the biggest. Move the largest disk into parity, or try Parity-Swap.',
...
export function deriveOperation(op: OperationState, opts: DeriveOpts = {}): PrimaryControl {
  ...
  switch (op.fsState) {
    case 'Started': {
      if (busy !== 0) {
        const what = busy === 1 ? 'a parity operation' : busy === 2 ? 'the mover' : 'a pool operation';
        return ctrl('Stop', false, `Disabled — ${what} is running.`);
      }
      return ctrl('Stop', true, null);
    }
    case 'Starting':   return ctrl('Starting…', false);
    case 'Stopping':   return ctrl('Stopping…', false);
    case 'Formatting': return ctrl('Formatting…', false);
    case 'Copying':    return ctrl('Cancel', true, 'Cancel will stop the parity copy.');
    case 'Clearing':   return ctrl('Cancel', true, 'Cancel will stop the disk clear.');
    case 'Stopped':    return deriveStopped(op, opts);
    default:           return ctrl('Start', false, 'Unknown array state.');
  }
}
