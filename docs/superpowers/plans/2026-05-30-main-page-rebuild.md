...
- [x] **Step 1:** `md-main-operation-panel.ts` — status orb (`mdColor`) + `fsState` label; Start/Stop/Cancel button from `deriveOperation`; effective-enabled logic that re-enables Start once overridable gates (confirm / encryption) are satisfied but keeps hard disables (config/ERROR) off; maintenance + confirmStart gates; Format + confirm (when `unmountableMask`). Wires to `actions`. Hosts the parity/spin/power sub-panels. Smoke tests: Stopped→Start enabled; Started→Stop POSTs cmdStop; DISABLE_DISK→disabled-until-confirm; ERROR→hard-disabled no gate; encrypted→disabled + fields rendered.
- [x] **Step 1b (encryption — Stopped state):** `md-main-encryption-fields.ts` — status label, Passphrase/Keyfile select, passphrase + show toggle, retype (when reformat), keyfile upload (FileReader→dataURL), **guarded "permit reformat"** (wrong-key only; default off; reveals an inline destructive warning + retype + a SECOND acknowledgement checkbox; `getKeyEntry().valid` stays false until ack + matching retype), and Delete-keyfile control. `getKeyEntry()` + `enc-change` event drive the panel's Start gate. Smoke tests confirm the guarded-reformat gate (invalid until ack + retype match).
- [x] **Step 2:** `md-main-parity-panel.ts` — Check(+correct)/Sync/Clear by `parity.action`; Pause/Resume/Cancel + progress bar (`pct`, pos/size) + error count while running; last-check summary.
- [x] **Step 3:** `md-main-spin-controls.ts` — Spin Up all / Spin Down all / Clear Stats; disabled while `busy != 0`.
- [x] **Step 4:** `md-main-power-panel.ts` — Move/Empty (when `moverEnabled` && started), Reboot, Shutdown (each `confirm()`-gated), safe-mode checkbox.
...
- [ ] **On-rig manual verification — DEFERRED to the first deploy (with Task 3 Step 8 / Task 11):** Stop/Start array, Spin down all, Clear Stats produce identical `emhttp` results to stock; parity start/cancel only if safe. **Confirm with user before first production deploy.**
