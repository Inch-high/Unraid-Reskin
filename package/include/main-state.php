...
function modernui_parse_ini_sections(string $path): array {
    ...
    foreach (file($path, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) as $line) {
        ...
        $pos = strpos($t, '=');
        if ($pos === false || $section === null) continue;
...
function modernui_map_status(string $status, string $fsStatus): string {
    if (strpos($status, '_MISSING') !== false) return 'missing';
    if ($status === 'DISK_DSBL' || $status === 'DISK_NP_DSBL') return 'disabled';
    if ($status === 'DISK_INVALID') return 'invalid';
    if ($status === 'DISK_WRONG')   return 'wrong';
    if ($status === 'DISK_NEW')     return 'new';
    if ($status === 'DISK_NP')      return 'notpresent';
    if ($fsStatus !== '' && stripos($fsStatus, 'Unmountable') === 0) return 'unmountable';
    return 'ok';
}
...
function modernui_map_orb(string $color): string {
    if (strpos($color, 'blink') !== false) return 'grey';
    if (strpos($color, 'green') === 0)  return 'green';
    if (strpos($color, 'yellow') === 0) return 'yellow';
    if (strpos($color, 'red') === 0)    return 'red';
    return 'grey';
}
...
function modernui_map_smart(array $d): string {
    if (trim((string)($d['critical'] ?? '')) !== '') return 'failed';
    $errors = modernui_int_or_null($d['numErrors'] ?? null) ?? 0;
    if ($errors > 0 || trim((string)($d['warning'] ?? '')) !== '') return 'warning';
    return 'healthy';
}
...
function modernui_derive_encryption(array $disks, array $var, array $poolNames): array {
    ...
    foreach ($disks as $d) {
        ...
        $auto = $fsType === 'auto';
        if ($luks || ($auto && strpos($defaultFsType, 'luks:') === 0)) $forced = true;
        if ($luks || $auto) {
    ...
    if (!$encrypt)      $mode = 'none';
    elseif ($forced)    $mode = 'enter-new';
    elseif ($missing)   $mode = 'missing-key';
    elseif ($wrong)     $mode = 'wrong-key';
    else                $mode = 'unlocked';
...
function modernui_parity_state(array $var): array {
    ...
    $verb = null;
    if (strpos($action, 'recon') !== false)      $verb = 'recon';
    elseif (strpos($action, 'clear') !== false)  $verb = 'clear';
    elseif (strpos($action, 'check') !== false)  $verb = 'check';
...
function modernui_main_state(array $disks, array $var, string $csrf = ''): array {
    ...
    foreach ($disks as $name => $d) {
        if ((string)($d['type'] ?? '') !== 'Cache') continue;
        $owner = null;
        ...
        if ($owner === null) continue;   // orphan member without a detectable leader
    ...
    $aSize = $aUsed = $aFree = 0; $haveFs = false;
