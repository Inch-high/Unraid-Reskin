...
function modernui_read_tags(): array {
    ...
    if (!isset($data['assignments']) || !is_array($data['assignments'])) $data['assignments'] = [];
    return $data;
}
...
function modernui_validate_folders(array $data): array {
    if (($data['version'] ?? null) !== 1) return ['ok' => false, 'error' => 'unsupported version'];
    $folders = $data['folders'] ?? null;
    if (!is_array($folders)) return ['ok' => false, 'error' => 'folders not array'];

    $seenIds = [];
    foreach ($folders as $f) {
        if (!is_array($f)) return ['ok' => false, 'error' => 'folder not object'];
        if (!modernui_is_valid_id($f['id'] ?? null)) return ['ok' => false, 'error' => 'bad folder id'];
        if (isset($seenIds[$f['id']])) return ['ok' => false, 'error' => 'duplicate folder id'];
        $seenIds[$f['id']] = true;
        if (!is_string($f['name'] ?? null) || trim($f['name']) === '') return ['ok' => false, 'error' => 'bad folder name'];
        if (strlen($f['name']) > 64) return ['ok' => false, 'error' => 'folder name too long'];
        if (!is_string($f['icon'] ?? null)) return ['ok' => false, 'error' => 'bad folder icon'];
        if (!modernui_is_valid_hex_color($f['color'] ?? null)) return ['ok' => false, 'error' => 'bad folder color'];
        $names = $f['containerNames'] ?? null;
        if (!is_array($names)) return ['ok' => false, 'error' => 'bad containerNames'];
        foreach ($names as $n) if (!is_string($n) || $n === '') return ['ok' => false, 'error' => 'bad container name'];
    }
    return ['ok' => true];
}
...
function modernui_validate_tags(array $data): array {
    if (($data['version'] ?? null) !== 1) return ['ok' => false, 'error' => 'unsupported version'];
    $tags = $data['tags'] ?? null;
    if (!is_array($tags)) return ['ok' => false, 'error' => 'tags not array'];

    $tagIds = [];
    foreach ($tags as $t) {
        if (!is_array($t)) return ['ok' => false, 'error' => 'tag not object'];
        if (!modernui_is_valid_id($t['id'] ?? null)) return ['ok' => false, 'error' => 'bad tag id'];
        if (isset($tagIds[$t['id']])) return ['ok' => false, 'error' => 'duplicate tag id'];
        $tagIds[$t['id']] = true;
        if (!is_string($t['name'] ?? null) || trim($t['name']) === '') return ['ok' => false, 'error' => 'bad tag name'];
        if (strlen($t['name']) > 32) return ['ok' => false, 'error' => 'tag name too long'];
        if (!modernui_is_valid_hex_color($t['color'] ?? null)) return ['ok' => false, 'error' => 'bad tag color'];
    }

    $assignments = $data['assignments'] ?? null;
    if (!is_array($assignments)) return ['ok' => false, 'error' => 'assignments not array'];
    foreach ($assignments as $containerName => $ids) {
        if (!is_string($containerName) || $containerName === '') return ['ok' => false, 'error' => 'bad assignment key'];
        if (!is_array($ids)) return ['ok' => false, 'error' => 'bad assignment value'];
        foreach ($ids as $tid) {
...
function modernui_maybe_migrate_folder_view2(): void {
    if (is_file(MODERNUI_DOCKER_FOLDERS)) return;
    if (!is_file(MODERNUI_FOLDERS_LEGACY)) return;
    ...
    foreach ($data as $legacyId => $entry) {
        ...
        if (!is_array($containerNames)) continue;
        ...
        if (!in_array($icon, $allowedIcons, true)) $icon = $defaultIcon;
