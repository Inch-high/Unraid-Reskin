...
foreach (['ArrayDevices.page', 'CacheDevices.page', 'BootDevice.page', 'ArrayOperation.page'] as $name) {
    ...
    assert(
        $table[$target] === MODERNUI_OVERLAY_DIR . '/usr/local/emhttp/plugins/dynamix/' . $name,
...
assert(
    strpos($ad, 'safemode') !== false && strpos($ad, 'disabled') !== false,
...
assert(
    strpos($ao, 'Nchan="device_list,disk_load,parity_list"') !== false,
...
assert(
    preg_match('/foreach \(modernui_main_overlay_table\(\) as \$target => \$overlaySrc\) \{\s*modernui_replace_file/', $install) === 1,
    'modernui_install must replace every Main overlay'
);

$upgrade = file_get_contents($root . '/package/include/upgrade.php');
assert(
    strpos($upgrade, 'modernui_main_overlay_table()') !== false,
...
assert(
    strpos($uninstall, 'modernui_main_overlay_table()') !== false,
    'uninstall must restore the Main pages'
);

$save = file_get_contents($root . '/package/include/save.php');
assert(
    strpos($save, 'modernui_replace_main_pages()') !== false
...
assert(
    preg_match('/action.*disable.*\}.*modernui_restore_main_pages\(\)/s', $save) !== 0
    || strpos($save, 'modernui_restore_main_pages();') !== false,
