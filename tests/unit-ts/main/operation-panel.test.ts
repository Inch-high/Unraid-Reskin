...
import type { MainPageState, OperationState, EncryptionState, EncryptionMode } from '../../../src/ts/main/types';

function enc(mode: EncryptionMode = 'none'): EncryptionState {
  return { required: mode !== 'none', mode, keyfilePresent: false, allowReformat: false, poolNames: ['cache'] };
}
function op(over: Partial<OperationState> = {}): OperationState {
  return {
    ...
    unmountableMask: '', encryption: enc(), moverEnabled: true, busy: 0, ...over,
  };
}
function state(o: Partial<OperationState> = {}): MainPageState {
  return {
    ...
    pools: [], boot: null,
    parity: { action: null, correcting: false, running: false, paused: false, posBytes: null, sizeBytes: null, pct: null, speed: null, errors: 0, corrected: null, last: null, scheduleEnabled: true },
    operation: op(o), serverVersion: '7.3.1', csrfToken: 'CSRF',
  };
}
...
describe('md-main-operation-panel', () => {
  ...
  it('encrypted (missing-key) → Start disabled, encryption fields rendered', async () => {
    const el = await mount(state({ fsState: 'Stopped', mdState: 'STOPPED', encryption: enc('missing-key') }));
    const btn = el.shadowRoot!.querySelector('button.action') as HTMLButtonElement;
    expect(btn.disabled).toBe(true);
...
describe('md-main-encryption-fields — guarded reformat', () => {
  ...
  it('valid passphrase makes entry valid (missing-key, no reformat)', async () => {
    ...
    pass.value = 'hunter2'; pass.dispatchEvent(new Event('input'));
  ...
  it('wrong-key reformat stays invalid until ack + matching retype', async () => {
    ...
    reformat.checked = true; reformat.dispatchEvent(new Event('change'));
    await (el as unknown as { updateComplete: Promise<unknown> }).updateComplete;
    const pass = el.shadowRoot!.querySelector('#enc-pass') as HTMLInputElement;
    pass.value = 'secret'; pass.dispatchEvent(new Event('input'));
    await (el as unknown as { updateComplete: Promise<unknown> }).updateComplete;
    const retype = el.shadowRoot!.querySelector('#enc-retype') as HTMLInputElement;
    retype.value = 'secret'; retype.dispatchEvent(new Event('input'));
