...
function remote(over: Partial<UnassignedRemote> = {}): UnassignedRemote {
  return {
    protocol: 'smb',
    name: '192.168.10.99_Backups',
    ip: '192.168.10.99',
    share: 'Backups',
    mountpoint: '/mnt/remotes/192.168.10.99_Backups',
    fsType: 'cifs',
    device: 'SMB_192.168.10.99_Backups',
    mounted: true,
    alive: true,
    readOnly: false,
...
describe('md-main-unassigned-card', () => {
  ...
  it('shows the empty-disks note and the advanced-ops footnote', async () => {
    const el = await mount({
      ...
      disks: [],
      remotes: [],
      historical: [{ serial: 'KINGSTON_X', device: 'dev1', mountpoint: 'dev1', standby: false }],
    });
    const txt = el.shadowRoot!.textContent ?? '';
  ...
  it('renders historical / previous devices', async () => {
    const el = await mount({
      ...
      remotes: [],
      historical: [
        { serial: 'KINGSTON_SNV3S1000G_X', device: 'dev1', mountpoint: 'dev1', standby: false },
      ],
    });
    const txt = el.shadowRoot!.textContent ?? '';
