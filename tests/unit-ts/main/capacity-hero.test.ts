...
function device(over: Partial<MainDevice> = {}): MainDevice {
  return {
    ...
    model: 'M', serial: 'S', status: 'ok', spin: 'active', spunDown: false, tempC: 38,
    numReads: 0, numWrites: 0, numErrors: 0, fsType: 'xfs', encrypted: false, luksState: null,
    sizeBytes: 12e12, fsSizeBytes: 12e12, fsUsedBytes: 9e12, fsFreeBytes: 3e12,
...
describe('md-main-capacity-hero', () => {
  const array: MainArray = {
    ...
    sizeBytes: 24e12, usedBytes: 18e12, freeBytes: 6e12, utilizationPct: 75,
  };
  ...
  it('reads Unprotected when parity is invalid and tints the bar high', async () => {
    ...
    expect((el.shadowRoot!.querySelector('.bar > span') as HTMLElement).classList.contains('high')).toBe(true);
  });
});
